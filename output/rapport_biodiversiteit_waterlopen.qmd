...
- **Aantal stress-tolerante soorten (nst)**: Deelmaatlat is aantal. In deze analyse als aandeel van totale aantal taxa dat gevoelig is aan verstoring (tolerantiescore \> 6; exclusief EPT).

- **Aantal EPT-soorten (ept)**: Idem als nst; het aandeel gevoelige taxa behorende tot de Ephemeroptera, Plecoptera en Trichoptera.

#### Functionele diversiteit en community gewogen trait waarden

[Code op Github](https://github.com/inbo/toestand-waterlopen/blob/main/source/functionele_diversiteit_mi.R)
...
De eigenschappen zijn geselecteerd uit de Tachet trait databank: temperatuur, dispersie, voortbeweging (locomotion), reproductie, saprobiteit, stroomsnelheid, trofisch status, respiratie, ph en substraat.

We berekenden functionele dispersie, richness en eveness voor 3 subsets van traits.

- alle bovenstaande traits

- waterkwaliteit: saprobiteit, trofisch status, respiratie, ph

- habitat: substraat, stroomsnelheid, voortbeweging

Enkel metingen met een "trait-coverage" \> 80% werden behouden voor analyse.
...
Om instabiliteit in de modellen te voorkomen, filteren we vooraf variabelen met een te hoge onderlinge correlatie ($r > 0.7$) of een hoge mate van multicollineariteit (op basis van de *Variance Inflation Factor*, VIF). Stikstofconcentratie, fosforconcentratie, lozingsvariabelen werden log-getransformeerd.
...
De individuele beste modellen voor biologie en de andere drivers worden samengevoegd in één overkoepelend SEM (**Piecewise SEM**). Dit netwerk maakt het mogelijk om zowel **directe effecten** (bijv. landgebruik $\rightarrow$ biologie) als **indirecte effecten** (bijv. landgebruik $\rightarrow$ nutriënten $\rightarrow$ biologie) te kwantificeren.

### Stap 4: Modelevaluatie en Verfijning

Het initiële pSEM wordt getoetst aan de hand van *directed separation* (d-sep) en de *Fisher's C-statistiek* om de fit te beoordelen. Op basis hiervan verfijnen we het model:

- **Pad-optimalisatie:** Ontbrekende maar ecologisch relevante paden die significant blijken, worden toegevoegd.

- **Gecorreleerde fouten:** Relaties tussen nauw verwante stuurvariabelen (zoals stikstof en fosfor) worden expliciet als gecorreleerde fouten gemodelleerd.
...
Missing values voor mi_nat_sv_beek. Vnl. nutrientvariabelen (zie "Nutriënten en (chemische) belasting").

![](figuren/missing_values_mi_nat_sv_beken.png)
...
Piecewise sem resultaat voor EPT - zeer stressgevoelige soorten met hoge o2 vereisten - voor de natuurlijke en sterk veranderde beken.

![](figuren/SEM_mi_nat_sv_beek_ept.png)

#### Functionele diversiteit en community gewogen trait waarden

![](figuren/trend_fd_subsets.png)

Deze trends zijn op basis van een gam model met gaussiaanse distributie. Probleem is dat de FD waarden rechts skewed zijn van verdeling. Ik zou de FD graag ook modelleren, maar weet niet zo goed met welke distributie dit best te doen; de residuelenplots geven sterk schending van de normaliteit etc aan.

![](figuren/CWM_trend_current_velocity.png)

Voorbeeld van een plot die de trend weergeeft van 1 van de traits gebruikt voor de berekening van FD. Hier stroomsnelheid. Echter voorzichtig te interpreteren, vnl voor de subsets voor rivieren, polders, rtnt (bovenlopen) en kunstmatig. Bij deze subsets is er wel wat jaarlijkse variatie in samplinglocatie; het samplingzwaartepunt verschuift sterk tussen jaren. Zie onderstaande figuur ter illustratie (legende nog aanpassen; grotere bolletjes is meer metingen in dat jaar):

![](figuren/ruimtelijke_drift_samples_per_jaar.png)
