...
if (!check_ecotox_availability()) {
  ...
  build_ecotox_sqlite(source = "data/ruw/pak/ecotox_ascii_09_11_2025")
}
...
# Selecteer pak (GEEN mengsels)
pak_target <- chemische_stoffen %>%
  filter(type == "PAK", subtype != "mengsel")

target_cas_list <- pak_target %>% pull(cas)
...
# --- Resultaat Bekijken ---
cat("\n📊 ANALYSE VOLTOOID\n")
cat("Totaal aantal pak in lijst:", nrow(pak_target), "\n")
cat("Aantal pak met gevonden EC50 data:", sum(!is.na(resultaat_pak$final_ec50_ug_L)), "\n")
cat("Dekking:", round(sum(!is.na(pak_ec50$final_ec50_ug_L)) / nrow(pak_target) * 100, 1), "%\n")

# Opslaan
save(pak_ec50, file = here("data", "verwerkt", "polluenten", "pak_ec50.rdata"))
