...
load(here("data", "verwerkt", "pesticiden_lc50.rdata"))
...
tu_dataset <- fc_data %>%
  ...
  inner_join(lc50_clean, by = c("parameter_symbool" = "stof_symbool")) %>%
...
# Chlck de lc50 waarden van de boosdoeners
chlck_drivers <- pesticiden_lc50 %>%
  filter(stof_naam %in% c("Malathion", "Diuron", "Imidacloprid")) %>%
  select(stof_naam, cas, final_lc50_ug_L, sensitive_group, sensitive_species)
