...
LC50_aggregated_mi <- data_LC50_final %>%
  ...
  summarise(
    LC50_species_geomean = geomean(LC50_ug_L),
    .groups = "drop"
  ) %>%
  ...
  group_by(cas) %>%
  summarise(
    final_LC50_ug_L = min(LC50_species_geomean),
    sensitive_species = latin_name[which.min(LC50_species_geomean)],
    sensitive_group = ecotox_group[which.min(LC50_species_geomean)],
    n_species_used = n(),
    .groups = "drop"
  )

# Koppel terug aan je originele lijst
pesticiden_LC50_mi <- pesticiden_target %>%
  left_join(LC50_aggregated_mi, by = "cas")
...
LC50_clean_mi <- pesticiden_LC50_mi %>%
  select(stof_symbool, cas, stof_naam, final_LC50_ug_L, sensitive_group, sensitive_species) %>%
...
tu_dataset_ruw_mi <- fc_data %>%
  ...
  # Alles uit fc_data dat niet in LC50_clean staat (zoals DDT t, Metalen, Nitraat)
  # verdwijnt hier automatisch.
  inner_join(LC50_clean_mi, by = c("parameter_symbool" = "stof_symbool")) %>%

  mutate(
    TU_individual = resultaat_ug_L / final_LC50_ug_L
  )
...
# We kijken in tu_dataset_ruw (dus stoffen die EN pesticide zijn EN een LC50 hebben)
...
#   select(meetplaats, monsternamedatum, parameter_symbool, resultaat_ug_L) %>%
#   # Koppel aan de MI-specifieke LC50 lijst
#   inner_join(pesticiden_LC50_mi, by = c("parameter_symbool" = "stof_symbool")) %>%
#   mutate(TU_mi = resultaat_ug_L / final_LC50_ug_L)
#
