...
# # ==============================================================================
#
# lc50_aggregated <- data_lc50_final %>%
...
#   summarise(
#     lc50_group_min = min(lc50_species_geomean),
#     most_sensitive_species = latin_name[which.min(lc50_species_geomean)],
#     .groups = "drop"
#   )
...
final_lc50_per_cas <- lc50_aggregated %>%
  group_by(cas) %>%
  summarise(
    # De absolute worst-case lc50 over alle groepen (Algen, Daphnia, Vis) heen
    final_lc50_ug_L = min(lc50_group_min),
    sensitive_group = ecotox_group[which.min(lc50_group_min)],
    sensitive_species = most_sensitive_species[which.min(lc50_group_min)]
  )

# Koppel terug aan je originele lijst
pesticiden_lc50 <- pesticiden_target %>%
  left_join(final_lc50_per_cas, by = "cas")

# --- Resultaat Bekijken ---
cat("\n📊 ANALYSE VOLTOOID\n")
cat("Totaal aantal pesticiden in lijst:", nrow(pesticiden_target), "\n")
cat("Aantal pesticiden met gevonden lc50 data:", sum(!is.na(resultaat_pesticiden$final_lc50_ug_L)), "\n")
cat("Dekking:", round(sum(!is.na(resultaat_pesticiden$final_lc50_ug_L)) / nrow(pesticiden_target) * 100, 1), "%\n")

# Opslaan
save(pesticiden_lc50, file = here("data", "verwerkt", "pesticiden_lc50.rdata"))
