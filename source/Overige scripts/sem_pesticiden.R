...
fc_lu_data_clean <- fc_lu_data %>%
  ...
  dplyr::select(groep, bekken, statuut, meetplaats, owl, ep_tw, ta_xw, ns_tw, sw_dw, mt_sw, mmif, mmif_20, n_t, p_h, ec_20, o2, p_t, landbouw_intens_afstr, hooggroen_afstr, hooggroen_oever, jaar, kjn, aantal_pesticiden_met_overschrijding, aantal_zware_metalen_met_overschrijding, Neerslag_som_10dagen, Neerslag_som_1jaar,
                ekc2_waterlichaam) %>%
...
m1 <- glmmTMB(data = fc_lu_data_clean,
              kjn ~ landbouw_intens_afstr_s + ekc2_waterlichaam_s + jaar_s + Neerslag_som_1jaar_s + Neerslag_som_10dagen_s + aantal_overstorten_500m_s + (1 | meetplaats),
              family = gaussian)

m3 <- glmmTMB(data = fc_lu_data_clean,
              p_t ~ landbouw_intens_afstr_s + ekc2_waterlichaam_s  + kjn_s + jaar_scaled + aantal_overstorten_500m + (1 | meetplaats),
              family = gaussian)

m4 <- glmmTMB(data = fc_lu_data_clean,
              o2 ~  landbouw_intens_afstr + p_t + kjn + aantal_pesticiden_met_overschrijding + aantal_overstorten_500m + scaled_neerslag_jaar + scaled_neerslag_piek +
                ekc2_waterlichaam + jaar_scaled + (1 | meetplaats),
              family = gaussian)

m5 <- glmmTMB(data = fc_lu_data_clean,
             aantal_pesticiden_met_overschrijding ~ landbouw_intens_afstr + hooggroen_oever + scaled_neerslag_jaar +  ekc2_waterlichaam + jaar_scaled + (1 | meetplaats),
              family = poisson)


# m2 <- glmmTMB(
#   mmif ~ kjn + landbouw_intens_afstr + scaled_neerslag_jaar + scaled_neerslag_piek + aantal_pesticiden_met_overschrijding + ekc2_waterlichaam + o2 + jaar_scaled + p_t + aantal_overstorten_500m  + (1 | bekken/meetplaats),
#   data = fc_lu_data_clean)

m2 <- glmmTMB(
  ept_prop ~ kjn + landbouw_intens_afstr + scaled_neerslag_jaar + aantal_pesticiden_met_overschrijding + ekc2_waterlichaam + o2 + jaar_scaled + p_t + aantal_overstorten_500m + (1 | bekken/meetplaats),
  weights = fc_lu_data_clean$ta_xw,
  data = fc_lu_data_clean,
  family =  binomial)

m2 <- glmmTMB(
  ta_xw ~ kjn + landbouw_intens_afstr + scaled_neerslag_jaar + aantal_pesticiden_met_overschrijding + ekc2_waterlichaam + o2 + jaar_scaled + p_t + aantal_overstorten_500m + (1 | bekken/meetplaats),
  data = fc_lu_data_clean,
  family =  poisson)

m2 <- glmmTMB(
  mt_sw_prop ~ kjn + landbouw_intens_afstr + scaled_neerslag_jaar + scaled_neerslag_piek + aantal_pesticiden_met_overschrijding + ekc2_waterlichaam + o2 + jaar_scaled + p_t  + aantal_overstorten_500m + (1 | bekken/meetplaats),
  family = ordbeta,
  data = fc_lu_data_clean)
...
m2 <- glmmTMB(
  nst_prop ~ kjn + landbouw_intens_afstr + scaled_neerslag_jaar + aantal_pesticiden_met_overschrijding + ekc2_waterlichaam + o2 + jaar_scaled + p_t + aantal_overstorten_500m + (1 | meetplaats),
  weights = fc_lu_data_clean$ta_xw,
  data = fc_lu_data_clean,
