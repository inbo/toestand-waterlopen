...
model_spear <- glmmTMB(data = data,
                      spear_pesticides ~ scale(log(resultaat_detectielimiet)) + scale(akker) + scale(aantal_pesticiden_met_overschrijding) + scale(hooggroen_afstr) + scale(jaar) + (1 | bekken/meetplaats),
                      family = ordbeta)
summary(model_spear)

model_mts <- glmmTMB(data = data,
                      mt_sw ~ scale(log(resultaat_detectielimiet)) + scale(spear_pesticides) + scale(aantal_pesticiden_met_overschrijding) + scale(hooggroen_afstr) + scale(jaar) + (1 | bekken/meetplaats),
                      family = gaussian)
summary(model_mts)
