...
# selecteren rijen waar slechts 1 match met vhag is

meetplaatsen_one_vhag <- koppeling_meetplaatsen_vhag %>% filter(n_vhag == 1)

meetplaatsen_toomany <- koppeling_meetplaatsen_vhag %>% filter(n_vhag > 1) %>% select(-MORFONAAM)
# unique(meetplaatsen_toomany$meetplaats) %>% length()

plot_punten1 <- meetplaatsen_toomany$meetplaats %>% unique()
plot_punten <- meetpunten_rtnt  %>% filter(meetplaats %in% plot_punten1)
plot_waterlopen1 <- meetplaatsen_toomany$vhag %>% unique()
plot_waterlopen <- morfotype %>% filter(VHAG %in% plot_waterlopen1)

mapview(plot_punten) + mapview(plot_waterlopen, zcol = "MORFONAAM")

nearest_river_index <- st_nearest_feature(plot_punten, plot_waterlopen) #verre punten met dubbele VHAG koppeling snappen naar dichtste


toomany_nearest <- plot_punten %>%
  mutate(morfotype = plot_waterlopen$MORFONAAM[nearest_river_index])


meetpunten_rtnt_morfotype1 <- bind_rows(meetplaatsen_one_vhag %>%
                                          rename(morfotype = MORFONAAM) %>%
                                          select(meetplaats, morfotype, vhag),
                                        toomany_nearest %>%
                                          select(meetplaats, morfotype, vhag))
...
punten_zonder_vhag <- punten_zonder$vhag
 wlas <- st_read(here("data", "ruw", "wlas", "Wlas.shp"))
wlas_na <- wlas %>% filter(VHAG %in% punten_zonder_vhag)#verre punten waarvan geen vhag kan gelinkt worden aan morfotype want ontbreekt in die laag. -> deze terug plotten en bekijken op kaart -> handmatig aangvuld

# mapview(punten_zonder) + mapview(morfotype)
...
mismatch_vhag <- punten_zonder %>% #meetplaatsen met een foute VHAG -> matcht niet met waterloop waar hij duidelijk op ligt, dus op basis van dichtste waterloop linken
...
nearest_river_index2 <- st_nearest_feature(mismatch_vhag, morfotype) #verre punten met dubbele VHAG koppeling snappen naar dichtste
mismatch_nearest <- mismatch_vhag %>%
  select(-morfotype, -vhag) %>%
...
meetpunten_rtnt_morfotype2 <- meetpunten_rtnt_morfotype1 %>%
  st_drop_geometry() %>%
  # select(-vhag) %>%
  drop_na(morfotype) %>%
  bind_rows(., morfotype_handmatig) %>%
  bind_rows(., mismatch_nearest) %>%
...
meetpunten_morfotype_mmif_herberekend <- mi_data_analyse %>%
  ...
  # st_drop_geometry() %>%
  right_join(., meetpunten_rtnt_morfotype2, by = "meetplaats") %>%
