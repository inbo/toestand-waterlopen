...
data_sem_clean0 <- mi_nat_sv %>%
  dplyr::select(groep, monsternamedatum, bekken, statuut, meetplaats, owl.x, ep_tw, ta_xw, ns_tw, sw_dw, mt_sw, mmif, mmif_20, n_t, ph, t_fc, ec_20_fc, o2_verz_fc, o2_fc, p_t, czv, natuur_oever, landbouw_intens_buffer, landbouw_intens_afstr, akker_afstr, hooggroen_afstr, hooggroen_oever, jaar, kjn, landbouw_intens_oever, aantal_pesticiden_met_overschrijding, aantal_zware_metalen_met_overschrijding, Neerslag_som_10dagen, Neerslag_som_1jaar, ekc2_waterlichaam, ekc2_traject, sinuositeit, profiel, aantal_overstorten_500m, score_overstorten_500m, overstorten_index, aantal_overstorten_weighted, overstorten_blootstelling_index, lozingen_rwzi_ie, lozingen_rwzi_p_t, lozingen_riool_ie, lozingen_riool_ie_conservative, lozingen_industrie_ie, verharding_afstr, spear_pesticides, tu_estimated,verharding_oever, spei6, n_extreme_3m, p_sum_7d, intensiteit_combo) %>%
  tidyr::drop_na() %>%
...
m4 <- glmmTMB(data = data_sem_clean,
              o2_verz_fc_s ~  intensiteit_combo_s + p_t_log + n_t_log  + overstorten_blootstelling_index_log +  spei6_s + n_extreme_3m_s + verharding_afstr_s + czv_log +
...
m5 <- glmmTMB(data = data_sem_clean,
              czv_log ~  intensiteit_combo_s + overstorten_blootstelling_index_log +  spei6_s + n_extreme_3m_s + verharding_afstr_s + p_t_log + n_t_log + ekc2_waterlichaam_s + lozingen_industrie_ie_log + lozingen_rwzi_ie_log + jaar_s + lozingen_riool_ie_log + (1 | meetplaats),
              family = gaussian)

m2 <- glmmTMB(
  mmif ~ n_t_log + intensiteit_combo_s + spei6_s + n_extreme_3m_s + verharding_afstr_s  + ekc2_waterlichaam_s + o2_verz_fc_s + jaar_s + p_t_log + overstorten_blootstelling_index_log + czv_log + lozingen_industrie_ie_log + lozingen_rwzi_ie_log + lozingen_riool_ie_log + (1 | meetplaats),
  family = ordbeta,
  data = data_sem_clean)
# #
# m2 <- glmmTMB(
...
#
# m2 <- glmmTMB(
#   mt_sw_prop ~ n_t_log + intensiteit_combo_s + spei6_s + n_extreme_3m_s + verharding_afstr_s  + ekc2_waterlichaam_s + o2_verz_fc_s + jaar_s + p_t_log + overstorten_blootstelling_index_log + czv_log + (1 | meetplaats),
#   family = ordbeta,
#   data = data_sem_clean)
#
# m2 <- glmmTMB(
#   sw_dw ~ n_t_log + intensiteit_combo_s + spei6_s + n_extreme_3m_s + verharding_afstr_s  + ekc2_waterlichaam_s + o2_verz_fc_s + jaar_s + p_t_log + overstorten_blootstelling_index_log + lozingen_industrie_ie_log + lozingen_rwzi_ie_log + czv_log + lozingen_riool_ie_log + (1 | meetplaats),
#   family = gaussian,
#   data = data_sem_clean)
#
# m2 <- glmmTMB(
#   nst_prop ~ n_t_log + intensiteit_combo_s + spei6_s + n_extreme_3m_s + verharding_afstr_s  + ekc2_waterlichaam_s + o2_verz_fc_s + jaar_s + p_t_log + overstorten_blootstelling_index_log + czv_log + (1 | meetplaats),
#   weights = data_sem_clean$ta_xw,
#   data = data_sem_clean,
