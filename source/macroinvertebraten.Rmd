...
kable(mi_data_analyse %>% st_drop_geometry() %>% filter(type == "RtNt") %>% group_by(type, categorie) %>% summarise(n()))
...
#stap 1: splitsen volgens waterlooptype -> kunstmatig vs niet (op het einde)
#stap 2: splitsen in ecoregio's -> polder, kempen, overig -> polder 1 soort scoring
#stap 3: splitsen voor kempen en 'overig' 
# -> categorie Bevaarbaar -> grote beek 
# -> categorie "Onbevaarbaar cat 1 -> grote beek
# -> categorie"Onbevaarbaar cat 2 -> kleine beek
# -> categorie"Onbevaarbaar cat 3 -> kleine beek
...
plot_model(voorbeeld_model2, type = "pred", terms = c("o2_verz"))
