...
library(lubridate)
library(mapview)
library(progress) # Nodig voor de nieuwe balk
...
time <- system.time({
  ...
  for (i in seq_len(nrow(mi_snapped))) {
    ...
    # Tick de progress bar bij elke iteratie
    pb$tick()
    ...
    if (nrow(candidates) > 0) {
      ...
      # >>> OPTIMALISATIE: Geen inner loop meer <<<
      # We berekenen de afstand van ALLE kandidaten naar het meetpunt in 1 keer.
      # Dit voorkomt dat je script vastloopt.

      d_matrix <- distances(
        g,
        v = as.character(candidates$node), # Van (upstream)
        to = as.character(mnode),          # Naar (downstream)
        mode = "out"                       # Volg de stroomrichting
      )

      # Resultaat is een matrix, omzetten naar vector
