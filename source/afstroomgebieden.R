...
if (st_crs(mi_meetpunten) != st_crs(fc_meetpunten)) {
  ...
  fc_meetpunten <- st_transform(fc_meetpunten, target_crs)
}
...
st_write(ow_meetpunten, dsn = here("data", "verwerkt", "ow_meetpunten.gpkg"), delete_dsn = T)
...
# =====================================================================
if (!file.exists(here("data", "verwerkt", "hydrologisch",
                      "ow_meetpunten_snapped_to_streams.shp"))) {

  snapped <- qgis_run_algorithm(
    "wbt:JensonSnapPourPoints",
    pour_pts = here("data", "verwerkt",
                    "ow_meetpunten.gpkg"),
    streams = stream_path,
    snap_dist = 300,
    output = here("data", "verwerkt", "hydrologisch",
...
# =====================================================================
if (!file.exists(here("data", "verwerkt", "hydrologisch",
                      "ow_meetpunten_watersheds_nested_1.tif"))) {
  watersheds_nested <- qgis_run_algorithm(
    ...
    pour_pts = here("data", "verwerkt", "hydrologisch",
                    "ow_meetpunten_snapped_to_streams.shp"),
    output = here("data", "verwerkt", "hydrologisch",
                  "ow_meetpunten_watersheds_nested.tif"),
    .quiet = TRUE
  )
}
...
# =====================================================================
if (!file.exists(here("data", "verwerkt", "hydrologisch",
                      "ow_meetpunten_watersheds_nested_all.gpkg"))) {
...
for (afstand in buffer_afstanden) {
  ...
  output_path <- here("data", "verwerkt", "hydrologisch",
                      paste0("ow_meetpunten_watersheds_buffered_", afstand, "m.gpkg"))
...
afstroomgebied_buffered_100m <- st_read(here("data", "verwerkt", "hydrologisch","ow_meetpunten_watersheds_buffered_100m.gpkg"))
