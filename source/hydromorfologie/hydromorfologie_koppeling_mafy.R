...
mafy_meetpunten <- mafy_data0 %>%
  select(meetplaats, vhas, owl) %>%
  unique()
...
hydmo_variabelen <- hydromorf_nieuw_traject %>%
  ...
  mutate(owl = as.character(owl_code))

meetpunten_prep <- mafy_meetpunten %>%
  mutate(owl = as.character(owl))

st_crs(mafy_meetpunten) == st_crs(hydmo_variabelen)

#---------------------------------------------------------------------------------------------------
# Stap 2: Koppeling meetpunten aan alle mogelijke trajecten via owl
#---------------------------------------------------------------------------------------------------

meetpunten_met_trajecten <- meetpunten_prep %>%
  st_drop_geometry() %>%
  left_join(hydmo_variabelen %>% select(owl, traj_code) %>%
              st_drop_geometry() %>%
              mutate(owl = as.character(owl)), by = "owl")

#---------------------------------------------------------------------------------------------------
# STAP 3: Hybride koppeling (owl eerst, daarna Afstand)
#---------------------------------------------------------------------------------------------------
...
hm_gekoppeld_totaal <- meetpunten_sf %>%
  ...
  purrr::map_dfr(function(punt) {
    ...
    # --- POGING 1: Match op basis van owl ---
    doel_owl <- as.character(punt$owl)
    trajecten_owl_subset <- hydmo_sf %>% filter(as.character(owl) == doel_owl)

    match_gevonden <- FALSE
    res <- NULL

    if (nrow(trajecten_owl_subset) > 0) {
      idx <- st_nearest_feature(punt, trajecten_owl_subset)
      res <- bind_cols(st_drop_geometry(punt),
                       st_drop_geometry(trajecten_owl_subset[idx, ]) %>% select(-owl))
      match_gevonden <- TRUE
    }
    ...
    if (!match_gevonden) {
      ...
      if (as.numeric(afstand) <= 25) {
        res <- bind_cols(st_drop_geometry(punt),
                         st_drop_geometry(hydmo_sf[idx_nearest, ]) %>% select(-owl))
...
save(hm_data_mafy, file = here("data", "verwerkt", "hm_data_mafy.rdata"))
